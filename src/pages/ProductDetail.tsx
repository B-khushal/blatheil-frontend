...
const ProductDetail = () => {
  ...
  const canonicalProductPath = `${basePath}/product/${product._id}`.replace(/\/\/+/, "/");
  const canonicalProductUrl = `${window.location.origin}${canonicalProductPath}`;
  // Redirect-safe share URL for hosts that force /index.html redirects.
